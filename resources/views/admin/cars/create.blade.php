...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">Tambah Mobil Baru</h1>
            <p>Tambahkan mobil baru ke dalam sistem</p>
        </div>
    ...
    <div class="bg-gray-800 rounded-lg shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route("admin.cars.store") }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="brand" name="brand" value="{{ old('brand') }}" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent bg-gray-900 text-white"
                ...
                <!-- Seat -->
                <div>
                    <label for="seat" class="block text-sm font-medium mb-2">Jumlah Kursi</label>
                    <input type="number" id="seat" name="seat" value="{{ old('seat') }}" required min="1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent bg-gray-900 text-white"
                ...
                <!-- Price Per Day -->
                <div>
                    <label for="price_per_day" class="block text-sm font-medium mb-2">Harga per Hari</label>
            ...
            <!-- Image -->
            <div>
                <label for="image" class="block text-sm font-medium mb-2">Foto Mobil</label>
                <input type="file" id="image" name="image" accept="image/*"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                <p class="text-sm text-gray-300 mt-1">Format: JPG, PNG, GIF. Maksimal 2MB.</p>
                @error('image')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
